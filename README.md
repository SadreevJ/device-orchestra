...
# E2E тест с виртуальным термометром
...
Этот тест имитирует работу **умного термометра** и показывает, как архитектура обрабатывает события в реальном времени без реального железа.

### 🌡️ Что демонстрирует виртуальный термометр:
...
```
✅ Система зафиксировала температурные измерения
✅ Обнаружен перегрев (температура > 28°C)  
✅ Автоматически отправлена команда охлаждения
✅ Устройство получило и обработало команду
✅ Все события записаны в логи
```

Этот тест **доказывает работоспособность всей архитектуры** — от создания устройства до обработки сложных сценариев взаимодействия компонентов.
