...
- ⏱️ **Экономия времени** - больше не собираете workflow из кусочков
...
```bash
ucdf status                           # Статус всех устройств
ucdf test <device_id>                 # Тестирование устройства  
ucdf debug <device_id>                # Интерактивная отладка
ucdf run-pipeline <file>              # Выполнение пайплайна
ucdf run-pipeline <file> --dry-run    # Симуляция без реального выполнения
```
...
UCDF использует структурированное логирование:
...
Логи автоматически сохраняются в `logs/ucdf_YYYYMMDD.log`.
